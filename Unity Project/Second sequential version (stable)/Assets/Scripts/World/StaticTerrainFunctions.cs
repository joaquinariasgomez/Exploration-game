...
public class StaticTerrainFunctions : MonoBehaviour {
    ...
    public static void AdjustBorderNormalsXZY(CubeSphere.Chunk thisChunk, int sqrtChunksPerFace, List<CubeSphere.Chunk> chunks)  //Modificar todas las normales de los bordes
    {
        int chunkSize = thisChunk.chunkSize;
        int reason = thisChunk.reason;
        int posX = thisChunk.posX;
            ...
            if (borderVertices[i] != new Vector3(0, 0, 0))
            {
                ...
                else    //Chunks externos
                {
                    ...
                    if (CubeSphere.GetRelativeChunkId(id, face) >= (sqrtChunksPerFace * (sqrtChunksPerFace - 1)))    //Chunk de arriba
                    {
                        if (i >= ((chunkSize / reason + 1) * (chunkSize / reason)))    //Normales de arriba deben de ser de otra cara
                        {
                            if (chunks[CubeSphere.GetChunkId(id, "up", face)].isActive && reason >= chunks[CubeSphere.GetChunkId(id, "up", face)].reason) borderNormals[i] = chunks[CubeSphere.GetChunkId(id, "up", face)].mesh.normals[(chunks[CubeSphere.GetChunkId(id, "up", face)].GetNumVertices() - 1 + ((chunkSize / chunks[CubeSphere.GetChunkId(id, "up", face)].reason + 1) * (chunkSize / chunks[CubeSphere.GetChunkId(id, "up", face)].reason))) - thisChunk.GetStartingI(CubeSphere.GetChunkId(id, "up", face), i, true)];
                        }/*
                        //Esquinas de arriba
                        if (CubeSphere.GetRelativeChunkId(id, face) == (sqrtChunksPerFace * (sqrtChunksPerFace - 1)))   //Chunk de arriba a la izquierda
                        {
                            if (i == ((chunkSize / reason + 1) * (chunkSize / reason)))  //Esquina arriba izquierda
                            {
                                if (chunks[CubeSphere.GetChunkId(id, "left", face)].isActive && reason >= chunks[CubeSphere.GetChunkId(id, "left", face)].reason) chunks[CubeSphere.GetChunkId(id, "left", face)].RebuildNormals((chunkSize / chunks[CubeSphere.GetChunkId(id, "left", face)].reason) * (chunkSize / chunks[CubeSphere.GetChunkId(id, "left", face)].reason + 1), borderNormals[i]);
                            }
                            if (i == ((chunkSize / reason + 1) * (chunkSize / reason + 1) - 1))  //Esquina arriba derecha
                            {
                                if (chunks[CubeSphere.GetChunkId(id, "up", face) + 1].isActive && reason >= chunks[CubeSphere.GetChunkId(id, "up", face) + 1].reason) chunks[CubeSphere.GetChunkId(id, "up", face) + 1].RebuildNormals(0, borderNormals[i]);
                    ...
                    if (CubeSphere.GetRelativeChunkId(id, face) < sqrtChunksPerFace)      //Chunk de abajo
                    {
                        if (i < (chunkSize / reason + 1))    //Normales de abajo deben de ser de otra cara
                        {
                            if (chunks[CubeSphere.GetChunkId(id, "down", face)].isActive && reason >= chunks[CubeSphere.GetChunkId(id, "down", face)].reason) borderNormals[i] = chunks[CubeSphere.GetChunkId(id, "down", face)].mesh.normals[(chunkSize / chunks[CubeSphere.GetChunkId(id, "down", face)].reason + 1) * (chunkSize / chunks[CubeSphere.GetChunkId(id, "down", face)].reason) + i];
                        }/*
                        //Esquinas de abajo
                        if (CubeSphere.GetRelativeChunkId(id, face) == 0)                        //Chunk de abajo a la izquierda
                        {
